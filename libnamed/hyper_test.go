...
func BenchmarkMapCap3072(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = make(map[uint16]*HyperTransport, 3072)
	}
}

func BenchmarkMapCap2(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = make(map[uint16]*HyperTransport, 2)
	}
}

func BenchmarkMapCap0(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = make(map[uint16]*HyperTransport)
	}
}
