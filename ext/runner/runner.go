...
func runOnceFuncs(ctx context.Context, ofs []*onceFunc, s string) error {
	...
	for i := range ofs {
		...
		go func(of *onceFunc) {
			start := time.Now()
			logEvent := xlog.Logger().Debug().Str("log_type", "runner").Str("op_type", s).Str("func", of.funcName)
			err := of.f()
			if err != nil {
				errCnt.Add(1)
			}
...
func (r *Runner) ExecDaemonStart(wg *sync.WaitGroup) error {
	...
	for i := range r.daemon {
		...
		go func(df *daemonFunc) {
			xlog.Logger().Debug().Str("log_type", "runner").Str("op_type", "daemon").Str("func", df.startFuncName).Msg("start ...")
			err := df.startFunc()
			xlog.Logger().Debug().Str("log_type", "runner").Str("op_type", "daemon").Str("func", df.startFuncName).Err(err).Msg("stoped")
			wg.Done()
		}(r.daemon[i])
	}
	return nil
...
func (r *Runner) ExecDaemonReload(timeout time.Duration) error {
	...
	for i := range r.daemon {
		wg.Add(1)
		go func(df *daemonFunc) {
			xlog.Logger().Debug().Str("log_type", "runner").Str("op_type", "daemon").Str("func", df.reloadFuncName).Msg("sending reload signal ...")
			err := df.reloadFunc()
			xlog.Logger().Debug().Str("log_type", "runner").Str("op_type", "daemon").Str("func", df.reloadFuncName).Err(err).Msg("sended reload signal")
			wg.Done()
		}(r.daemon[i])
	}
	finishChan := make(chan struct{})
...
func (r *Runner) ExecDaemonStop(timeout time.Duration) error {
	...
	for i := range r.daemon {
		swg.Add(1)
		go func(df *daemonFunc) {
			xlog.Logger().Debug().Str("log_type", "runner").Str("op_type", "daemon").Str("func", df.stopFuncName).Msg("sending stop signal ...")
			err := df.stopFunc()
			xlog.Logger().Debug().Str("log_type", "runner").Str("op_type", "daemon").Str("func", df.stopFuncName).Err(err).Msg("sended stop signal")
			swg.Done()
		}(r.daemon[i])
	}
